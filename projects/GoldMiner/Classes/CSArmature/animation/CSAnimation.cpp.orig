...
void Animation::updateHandler()
{
	...
	if(index_frame_event != getCurrentFrameIndex()){
		index_frame_event =getCurrentFrameIndex();
		//m_pArmature->onMovementEvent(IN_FRAME, m_strMovementID.c_str());
		MovementEventSignal.emit(m_pArmature, IN_FRAME, m_strMovementID.c_str());
	}
	updateFrameData(m_fCurrentPercent);
}
